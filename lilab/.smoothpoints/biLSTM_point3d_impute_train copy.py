...
def root_mean_squared_error(y_true, y_pred, pos): 
    return np.sqrt(np.nanmean(np.square(y_true[pos] - y_pred[pos])))

def root_mean_squared_error3d(y_true, y_pred, pos): 
    diff = y_true[pos] - y_pred[pos]
    diff3d = diff.reshape(-1, 3)
    return np.nanmean(np.sqrt(np.sum(np.square(diff3d), axis=1)))
...
class Model(nn.Module):
    ...
    def forward(self, x):
        batchsize = x.shape[0]
        hidden = (torch.randn(self.num_layers * 2, batchsize, self.hidden_size),
                  torch.randn(self.num_layers * 2, batchsize, self.hidden_size))
        x = self.linerinput(x)
        x = self.relu(x)
...
def train_model(model, train_dataloader):
    ...
    out_checkpoint = osp.join(osp.dirname(__file__), 'modelbilstm.pth')
