...
from dannce.engine.generator_cxf import (
    DataGenerator_3Dconv_torch_video_canvas_faster_single as DataGenerator_3Dconv_torch_video_single,
)
...
from lilab.multiview_scripts_dev.s1_ballvideo2matpkl_full_realtimecam import (
    ...
    mid_gpu,
    preview_resize,
    MyWorker as OldMyWorker,
)

com3d_config_dict = {
    6: "/home/liying_lab/chenxinfeng/DATA/mmpose/hrnet_w32_coco_ball_512x512.py",
    10: "/home/liying_lab/chenxinfeng/DATA/mmpose/res50_coco_ball_512x512.py",
    4: r"E:\mmpose\res50_coco_ball_512x320_ZJF.py",
}

voxel_config_dict = {
    4: r"/home/liying_lab/chenxinfeng/DATA/dannce/demo_single/rat14_1280x800x4_mono/params.yaml"
}
...
class DataLoader_video_canvas_faster_single(DataGenerator_3Dconv_torch_video_single):
    ...
    def set_video(self, noneuse, gpu, pkldata):
        ...
        coms_3d = np.array([[-20, -20, -10], [20, 20, 10]])
    ...
    def quary_voxel(self, ims_pannel, com_3d):
        ...
        for i, ID in enumerate(range(self.nclass)):
            ...
            [X_each, xgrid_roi_each], y_3d_each = self.quary_gridsample_by_com3d(
                com_3d, ims
            )
            X.append(X_each)
...
class MyWorker(OldMyWorker):
    def __init__(
        ...
        super().__init__(
            com3d_config, video_file, com3d_checkpoint, ballcalib, views_xywh
        )
...
def post_cpu(camsize, heatmap, center, scale, views_xywh, img_preview, calibobj):
    ...
    preds = transform_preds(preds, center, scale, [W, H], use_udp=False)
...
if __name__ == "__main__":
    ...
    com3d_config, com3d_checkpoint, ballcalib = (
        arg.com3d_config,
        arg.com3d_checkpoint,
        arg.ballcalib,
    )
    if com3d_config is None:
        com3d_config = com3d_config_dict[arg.pannels]
    if com3d_checkpoint is None:
        com3d_checkpoint = findcheckpoint_trt(
            com3d_config, trtnake="latest.full_fp16.engine"
        )
    ...
    if voxel_checkpoint is None:
        voxel_checkpoint = (
            os.path.dirname(com3d_checkpoint)
            + "/DANNCE/train_results/MAX/latest_fp16.engine"
        )
