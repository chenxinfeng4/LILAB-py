...
def main(modelfile, batchsize):
    ...
    input_layer = Input(shape=(64, 64, 64, 9), batch_size=None, name="input_1")
    output = model(input_layer)
    output = Reshape((-1, output.shape[-1]))(output)
    ...
    onnx_file = osp.splitext(modelfile)[0] + (
        ".idx.onnx" if batchsize <= 1 else f"_batch{batchsize}.idx.onnx"
    )
