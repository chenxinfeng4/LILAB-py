...
class MyWorker(OldMyWorker):
    def compute(self, args):
        ...
        with torch.cuda.device(self.cuda):
            ...
            heatmap = mid_gpu(trt_model, img_NCHW, input_dtype)
            kpt3d, kpt2d = post_cpu(
                self.camsize, heatmap, center, scale, views_xywh, img_preview, calibobj
            )
            keypoints_xyz_ba.append(kpt3d)
            keypoints_xyp.append(kpt2d)
        ...
        keypoints_xyp = np.array(keypoints_xyp).transpose(
            1, 0, 2, 3
        )  # (nview, T, 1, 3)
        ...
        coms_3d = (
            ...
            .fit_transform(keypoints_xyz_ba.reshape(-1, 3))
            .reshape(keypoints_xyz_ba.shape)
        )  # (T,1,3)
        ...
        info = {
            "vfile": self.video_file,
            "nview": len(self.views_xywh),
            "fps": self.vid.fps,
        }
