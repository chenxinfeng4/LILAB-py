...
# %%
boneindex = [(0,1), (0,2), (2,3), (3,6), (3,7), (3,8), (6,7),(8,9),
            (4,5),(0,5),(10,11),(12,13),(5,12),(6,8),(10,12),(10,11),
            (12,13),(8,12)]
boneindex = np.array(boneindex)

# %%
def get_bonelength(pts3d):
    bonelength = np.linalg.norm(pts3d[..., boneindex[:,0], :] - pts3d[..., boneindex[:,1], :], axis=-1)
    return bonelength
...
# %% 按夹角
boneangleindex = [(0,1,3), (0,3,2), (3,6,4), (3,4,8), 
                  (4,10,5), (4,5,12), (6,10,4), (8,4,12)]
...
for i in tqdm.tqdm(range(min_iter)):
    ...
    frame = cv2.putText(frame, "{:4.1f}".format(pvals[0]), (20,150), cv2.FONT_HERSHEY_SIMPLEX, 2, bgr_font_color[0], 2)
    frame = cv2.putText(frame, "{:4.1f}".format(pvals[1]), (20,220), cv2.FONT_HERSHEY_SIMPLEX, 2, bgr_font_color[1], 2)
    vid_out.write(frame)
vid_out.release()
vid.release()