...
def create_clips(matcalibpkl_l, vnake_l, paramdict):
    pklfile = '/mnt/liying.cibr.ac.cn_Data_Temp/multiview_9/zhongzhenchao/24FP-behCluster/2407FP-total/representitive_k36_filt_perc84/bhvTransPoints/wt_manualLabels_starts.pkl'
    with open(pklfile, 'rb') as f:
        embedCenterFrames = pickle.load(f)
    assert {'video_nake', 'Start (s)'}.issubset(embedCenterFrames.keys())

    df_records = pd.DataFrame()
    df_records['vnake'] = embedCenterFrames['video_nake']
    df_records['center_frame'] = (embedCenterFrames['Start (s)']*30).values.astype(int)
    df_records['startFrame'] = df_records['center_frame'] - 12
    df_records['isBlack'] = True
    df_records2 = df_records.copy()
    df_records2['isBlack'] = False
    df_records = pd.concat([df_records, df_records2], ignore_index=True)
...
def predict_cluster_labels(project_dir, trt_model, df_records, feat_clips):
    ...
    outpkl = project_dir + '/wt_manualLabels_starts.clippredpkl'
