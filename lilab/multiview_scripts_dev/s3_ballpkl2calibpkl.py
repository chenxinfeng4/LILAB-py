...
def main_calibrate(ballfile, ba_poseinitfile=ballcalibfile):
    ...
    b0_landmarksto3d(ba_poses, landmarks_move_xy, True, setup, background_img)
    ...
    assert 0.5 < zscale < 2
    param = {}
    param['kargs'], fun_ba2global, fun_global2ba, param['strfun_ba2global'], param['strfun_global2ba'] = a5_global_registrate_short(
            zscale)
    error_global = fun_ba2global(ba_points_global_xyz['points_3d']) - landmarks_global_mm['landmarks_global']
    error_global_sort = sorted(np.linalg.norm(error_global, axis=1).round(1).tolist())
    print('Global Markers Error {}(mm)'.format(error_global_sort))
