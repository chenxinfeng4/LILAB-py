# python -m lilab.multiview_scripts_dev.s2_matpkl_filter xxx.matpkl --iframe x --iview x
import argparse
import pickle 
import numpy as np
...
#%%错误点的坐标
def main(pklfile:str, iframe:int, iview:int):
    with open(pklfile, 'rb') as f:
        pkldata = pickle.load(f)
        
    nview, nframe,nnumber, ncoord = pkldata['keypoints'].shape #(nview, nframe, 1,3)
    assert iframe<nframe and iview<nview and (nnumber, ncoord)==(1,3)

    trace = pkldata['keypoints'][iview, iframe]
    distance = np.squeeze(np.linalg.norm(pkldata['keypoints'][iview] - trace, axis=-1))
    iframe_errors = distance < 10
    n_error = iframe_errors.sum()
    if n_error:
        print('%d outliers removal'%(n_error))
    pkldata['keypoints'][iview, iframe_errors] = np.nan
...
if __name__=='__main__':
    ...
    main(args.pklfile, args.iframe, args.iview)
