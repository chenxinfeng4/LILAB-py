...
from lilab.mmpose_dev.refine_tripleball import sort_peaks

num_gpus = min([torch.cuda.device_count(), 4])

config_dict = { 9:'/home/liying_lab/chenxinfeng/DATA/mmpose/res50_coco_ball_512x320_tripleball.py'}
...
def get_max_preds_triple(heatmaps:torch.Tensor):
    ...
    peaks = np.zeros((N, K, 3, 3), dtype=np.float32)
    for i in range(N):
        for k in range(K):
...
def post_cpu(camsize, heatmap, center, scale, views_xywh, img_preview, calibobj):
    ...
    maxvals[:] = np.min(maxvals, axis=1, keepdims=True)  #tripleball 共享最小 P 值
    keypoints_xyp = np.concatenate((preds, maxvals), axis=-1) #(N, K, xyp)
    keypoints_xyp = np.array([sort_peaks(k) for k in keypoints_xyp]) #tripleball 排序
