...
multiview_template_path = (
    "/home/liying_lab/chenxinfeng/ml-project/multiview_scripts/multiview_template"
)
...
def on_crop_video():
    with use_scope("ballglobal_messages", clear=True):
        ...
        os.system(
            f'ffmpeg -i "{ballvideopath}" -ss {selecttimestr} -c:v copy -y "{outballvideopath}"'
        )
...
def on_csv_to_landmarkjson():
    with use_scope("ballglobal_messages", clear=True):
        ...
        put_text(
            """python -m lilab.multiview_scripts.dlcBall_2_landmarks \
            $BALL_MOVE \
...
def on_copy_configs():
    ...
    templates = [
        "setup.json",
        "intrinsics_rational.json",
        "landmarks_global_world.json",
        "filenames.json",
        "ba_config_rational.json",
    ]
...
def on_compute_relative_poses():
    ...
    with use_scope("ballglobal_messages", clear=True):
        put_text(
            """python  -m multiview_calib.scripts.compute_relative_poses_robust \
	-s setup.json \
...
def on_concatenate_relative_poses():
    ...
    with use_scope("ballglobal_messages", clear=True):
        put_text(
            """python  -m multiview_calib.scripts.concatenate_relative_poses \
	-s setup.json \
...
def on_bundle_adjustment():
    ...
    with use_scope("ballglobal_messages", clear=True):
        put_text(
            """python  -m multiview_calib.scripts.bundle_adjustment \
	-s stup.json \
	-i intrinsics_rational.json \
...
def on_global_landmarks_to_point3d():
    ...
    mc.main(
        landmarks="../ballglobal/landmarks_global_pixel.json",
    ...
    with use_scope("ballglobal_messages", clear=True):
        put_text(
            """python -m lilab.multiview_scripts.landmarks2point3d \
...
def on_global_registration():
    ...
    with use_scope("ballglobal_messages", clear=True):
        put_text(
            """python  -m multiview_calib.scripts.global_registration \
	-s setup.json \
...
def app(parent=None):
    ...
    with use_scope(parent):
        ...
        pin.put_input(
            "start_time", label="The start time of ball", placeholder="00:00:01"
        )
