...
rat_cls = ['rat1', 'rat2', 'rat3', 'rat4']

# %%
labelme_dir = '/home/liying_lab/chenxinfeng/DATA/mmpose/data/rat_keypoints/ww_rat_kpt_1280x800_0515/'
labelme_jsons = glob.glob(os.path.join(labelme_dir, '*.json'))
...
for labelme_json in labelme_jsons:
    ...
    kpt_labels = [label.split('_')[1] for label in labels]
    kpt_positions = [shape['points'][0] for shape in shapes]
    
    # 3. keypoint info to annotations
    for key, group in groupby(zip(rat_labels, kpt_labels, kpt_positions), key=lambda a:a[0]):
        group = list(group)
        kpt_list = [[0,0,0] for _ in range(len(keypoint_cls))]
        for rat_label, kpt_label, kpt_position in group:
...
# 4. save coco json
out_coco_json = os.path.join(labelme_dir, 'rat_coco.json')
