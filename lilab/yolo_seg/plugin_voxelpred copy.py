...
def infer_dannce_max_trt(
    ...
    vidout = ffmpegcv.VideoWriterNV('/mnt/liying.cibr.ac.cn_Data_Temp/multiview_9/chenxf/out.mkv', codec='h264', fps=30)
...
def post_cpu(pred, X_grid, iframe, pannel_preview, calibPredict:CalibPredict, vidout):
    ...
    for j in range(pred.shape[0]):
        preds = pred[j].float()
        pred_max = preds.max(0).values.max(0).values.max(0).values.cpu().numpy()
        ...
        pred_max_l.append(pred_max)
        com_3d_l.append(com_3d)
    com_3d_l = np.array(com_3d_l).astype(np.float32)
    coord_l = np.array(coord_l)
    pred_max_l = np.array(pred_max_l)
    p3d = np.moveaxis(coord_l, 2, 1).astype(np.float32) #(nclass, K, 3)
    return
    com_2d_l = calibPredict.p3d_to_p2d(com_3d_l)[ipannel].astype(np.int32)
    ipannel = 1
    coord_2d = calibPredict.p3d_to_p2d(p3d)[ipannel]
