...
from lilab.yolo_seg.common_variable import (
    NFRAME, get_numpy_handle, create_shared_arrays_dannce, get_numpy_handle_dannce)
...
def get_vidout():
    vidout = FFmpegWriterNoblock(ffmpegcv.VideoWriterStreamRT,'rtsp://10.50.60.6:8554/mystream_behaviorlabel_result')
    return vidout
...
def video_generator_worker(params:Dict, ba_poses:Dict, model_file:AnyStr,
    ...
    # share array buffer
    numpy_dannce_X, numpy_dannce_Xgrid, numpy_dannce_imgpreview = get_numpy_handle_dannce(
    ...
    numpy_imgNKHW, numpy_com2d, numpy_previ, numpy_timecode = get_numpy_handle(
        shared_array_imgNKHW, shared_array_com2d, shared_array_previ, shared_array_timecode)
    valid_generator.set_video(coms_3d_fake, calibPredict,
                              numpy_imgNKHW, numpy_com2d, numpy_previ,
                              q, lock)
...
def dannce_predict_video_trt(params:Dict, ba_poses:Dict, model_file:AnyStr,
    ...
    _, _, _, numpy_timecode = get_numpy_handle(
        shared_array_imgNKHW, shared_array_com2d, shared_array_previ, shared_array_timecode)
    share_dannce_X, share_dannce_Xgrid, share_dannce_imgpreview = create_shared_arrays_dannce()
    numpy_dannce_X, numpy_dannce_Xgrid, numpy_dannce_imgpreview = get_numpy_handle_dannce(
        share_dannce_X, share_dannce_Xgrid, share_dannce_imgpreview)
    ...
    process = ctx.Process(target=video_generator_worker, args=(
    ...
    iter_process = tqdm.tqdm(itertools.count(), 
                            desc='3D_poses',
                            position=1)
    ...
    for iframe in iter_process:
        ...
        iter_process.set_description('[3D Pose] q={:>2}, i={:>4}, delay={:>3}'.format(q_dannce.qsize(), idx, dt_str))
