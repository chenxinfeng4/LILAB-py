...
def get_vidin():
    ...
    vidout = FFmpegWriterNoblock(ffmpegcv.VideoWriterStreamRT, 'rtsp://10.50.60.6:8554/mystream_preview', pix_fmt='gray')
    return vid, vidout
...
def main(shared_array_imgNNHW:SynchronizedArray,
    ...
    numpy_imgNKHW, numpy_com2d, numpy_previ, numpy_timecode = get_numpy_handle(
            shared_array_imgNNHW, shared_array_com2d, shared_array_previ, shared_array_timecode)
    ...
    with torch.cuda.device('cuda:0'):
        ...
        def pre_cpu(vid, vidout):
            ...
            frame_preview2 = np.ascontiguousarray(frame[0:pannelWH[1]:2, 0:pannelWH[0]:2])
        ...
        for idx, _ in enumerate(iter_process):
            ...
            iter_process.set_description('[Ins Seg] q={:>2}, i={:>4}, delay={:>3}'.format(q.qsize(), idx%100, dt_str))
