...
def convert(annFile):
    ...
    for imgId, frame_out in zip(imgIds, outdata):
        ...
        for i, anno in enumerate(annos):
            ...
            if np.sum(mask) <= 4:
                # ignore this mask
                frame_out[0][0].append([])
            else:
                x, y, w, h = cv2.boundingRect(mask.astype(np.uint8))
                pval = 1
                frame_out[0][0].append(np.array([x, y, x+w, y+h, pval]))

            frame_out[1][0].append(maskUtils.encode(
                    np.array(mask[:,:,np.newaxis], order='F', dtype=np.uint8))[0])
