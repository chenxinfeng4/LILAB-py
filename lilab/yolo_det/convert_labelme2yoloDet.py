'''
python -m lilab.yolo_det.convert_labelme2yoloDet --json_dir . --val_size 0.2 --target_dir /home/liying_lab/chenxinfeng/DATA/ultralytics/data/ball_rg/ball_20230115
...
def copy_files(any_labeling_data_path, target_dir, _label_id_map):
    img_l = glob.glob(osp.join(any_labeling_data_path, 'images/*/*.jpg')) + \
            glob.glob(osp.join(any_labeling_data_path, 'images/*/*.png'))
    ...
    labels_train = osp.join(target_dir, 'labels', 'train')
    labels_val = osp.join(target_dir, 'labels', 'val')
    ...
    with open(yaml_path, 'w+') as yaml_file:
        yaml_file.write(f'train: {images_train}\n')
        yaml_file.write(f'val: {images_val}\n\n')
        yaml_file.write(f'nc: {len(_label_id_map)}\n\n')
        
        names_str = ','.join(_label_id_map.keys())
...
if __name__ == '__main__':
    ...
    any_labeling_data_path = convertor._save_path_pfx
    copy_files(any_labeling_data_path, args.target_dir, convertor._label_id_map)
