...
import pickle
import numpy as np
from multiview_calib.bundle_adjustment_scipy import (pack_camera_params, unpack_camera_params, undistort_points, triangulate, project_points)
import itertools
import argparse
...
def project_points_short(views, poses, keypoints_xyz, image_shape=None):
    ...
    if image_shape:
        ...
        mask_inside = lambda proj: np.logical_and.reduce([proj[:,0]>0, proj[:,0]>0])
...
def build_points_short(views, poses, keypoints_xy):
    orginal_shape = keypoints_xy.shape
    assert orginal_shape[-1] == 2
    landmarks = keypoints_xy.reshape(orginal_shape[0], -1, orginal_shape[-1])
    landmarks = {i: landmarks[i] for i in range(orginal_shape[0])}
    p3d = build_input_short(views, poses, landmarks)
    keypoints_xyz_baglobal = p3d.reshape(*orginal_shape[1:-1], 3)
    return keypoints_xyz_baglobal
