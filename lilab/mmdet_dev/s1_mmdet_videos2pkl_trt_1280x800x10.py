# python -m lilab.mmdet_dev.s1_mmdet_videos2pkl_trt_1280x800x10  A.mp4 [CONFIG] [CHECKPOINT]
# python -m lilab.mmdet_dev.s1_mmdet_videos2pkl_trt_1280x800x10  A/B/C
import argparse
import mmcv
import numpy as np
import tqdm
...
# class MyWorker():
    ...
    def compute(self, args):
        with torch.cuda.device(self.cuda), torch.no_grad():
            ...
            resize_init = (800, 608)
            img_metas = prefetch_img_metas(model.cfg, resize_init)
            resize_wh = img_metas['pad_shape'][1::-1]
            assert resize_wh == resize_init
...
if __name__ == '__main__':
    ...
    # detpkls = [worker.compute(arg) for arg in args_iterable]
