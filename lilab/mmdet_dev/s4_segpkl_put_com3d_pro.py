...
def center_of_mass_cxf(input):
    a_x, a_y = (
        np.sum(input, axis=0, keepdims=True),
        np.sum(input, axis=1, keepdims=True),
    )
    a_all = np.sum(a_x)
...
def convert(segpkl, calibpkl):
    ...
    coms_2d_smooth = calibPredict.p3d_to_p2d(
        coms_3d_smooth
    )  # nviews, nframes, nclass, 2
    pkl_data.update(
        {
