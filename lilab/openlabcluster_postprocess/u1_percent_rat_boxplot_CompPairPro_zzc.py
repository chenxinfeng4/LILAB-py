...
#%%
##path
project = '/mnt/liying.cibr.ac.cn_Data_Temp/multiview_9/zhongzhenchao/24behv_s3ms/representitive_k36_filt_perc85_shank3'
...
def define_group(rat_info, video_info, df_labnames=None):
    ...
    firstrat_geno_dict = dict(rat_info[['animal', 'first_ratgeno']].values)
    rows_list = []
    for i in range(video_info.shape[0]):
        record_now = video_info.iloc[i]
        video_nake = record_now['video_nake']

        animal_ratid, partner_ratid, video_day = record_now['animal'], record_now['partner'], record_now['video_day']
        ...
        firstrat_geno = firstrat_geno_dict[black_ratid]
        # rows_list.append([black_ratid, white_ratid, group_name, group_geno_name, video_nake, True])
        rows_list.append([black_ratid, white_ratid, group_name, group_geno_name, video_nake, True, video_day,firstrat_geno])
        ...
        firstrat_geno = firstrat_geno_dict[white_ratid]
        # rows_list.append([white_ratid, black_ratid, group_name, group_geno_name, video_nake, False])
        rows_list.append([white_ratid, black_ratid, group_name, group_geno_name, video_nake, False, video_day,firstrat_geno])

    # df_group = pd.DataFrame(columns=['first_ratid', 'partner_ratid', 'group', 'group_geno', 'video_nake', 'is_blackfirst'],
    #                         data=rows_list)
    df_group = pd.DataFrame(columns=['first_ratid', 'partner_ratid', 'group', 'group_geno', 'video_nake', 'is_blackfirst','video_day','first_ratgeno'],
                        data=rows_list)
...
def plot_box_data(df_labnames, df_group_x_freq, hue_order, hue_legend, savefigpath=None):
    ...
    df_group_x_freq['freq1'] = df_group_x_freq['freq'] * 100
    plt.figure(figsize = (15,15))
    sns.boxplot(y='lab_names', x='freq1', hue='group', 
                hue_order=hue_order,
                data= df_group_x_freq, ##data=df_group_x_freq, 使用percentage横坐标，扩大原有坐标
                width=0.45,
    ...
    plt.xlabel('Duration(Percentage)', fontsize=20)
...
# %%
######### use 3 sigma to delete outflies
def extreme_3sigma(dt):
    n=3
    mean = dt.mean()           # 截面数据均值
    std = dt.std()             # 截面数据标准差
    dt_up = mean + n*std       # 上限
    dt_down = mean - n*std     # 下限
    return np.clip(dt, dt_down, dt_up)   # 超出上下限的值，赋值为上下限
