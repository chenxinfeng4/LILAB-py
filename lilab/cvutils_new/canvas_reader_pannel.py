...
class CanvasReaderPannel(object):
    ...
    def __init__(self, video_path, segment_path=None, gpu=0, dilate=True):
        ...
        if len(views_xywh) == 6:
            vid = ffmpegcv.VideoReaderNV(
                video_path, gpu=gpu, pix_fmt="gray", crop_xywh=[0, 0, 800 * 3, 600 * 2]
            )
        elif len(views_xywh) in (4, 9, 10):
            vid = ffmpegcv.VideoReaderNV(video_path, gpu=gpu, pix_fmt="gray")
...
class CanvasReaderPannelMask(object):
    ...
    def __init__(self, video_path, segment_path=None, gpu=0, dilate=True):
        ...
        views_xywh = pkl_data["views_xywh"]
        if len(views_xywh) == 6:
            vid = ffmpegcv.noblock(
                ffmpegcv.VideoReaderNV,
                video_path,
                gpu=gpu,
                pix_fmt="gray",
            ...
            vid = ffmpegcv.noblock(
                ffmpegcv.VideoReaderNV, video_path, gpu=gpu, pix_fmt="gray"
            )
            h_w = (800, 1280)
        ...
        self.dataset = MaskDataset(
            pkl_data, self.dilate, self.h_w, self.nclass, self.nview
        )
    ...
    def read_canvas_mask_img_out_old(self):
        ...
        for iview in range(self.nview):
            for iclass in range(self.nclass):
                mask_out_imgs[iclass][iview] = (
                    imgpannels[iview] * maskiclass_canvas_a[iclass][iview][:, :, None]
                )
    ...
    def read_canvas_mask_img_out_single(self, imgpannels=None):
        ...
        for iview in range(self.nview):
            for iclass in range(self.nclass):
                ...
                # Case 4, single cpu
                mask_out_imgs[iclass][iview] = (
                    imgpannels[iview] * maskiclass_canvas[iclass][iview][:, :, None]
                )
                # Case 5, multple cpu
...
def cv2_resize(img, size, interpolation=cv2.INTER_NEAREST):
    ...
    if quary not in _map_dict:
        ...
        hw_coord_ravel = np.ravel_multi_index(
            hw_coord2d, dims=img.shape, order=order
        ).astype(np.int64)
...
class MaskDataset:
    ...
    def read_mask_only_old(self):
        ...
        maskiclass_canvas_a = [
            [None for _ in range(self.nview)] for _ in range(self.nclass)
        ]
        ...
        maskiclass_canvas = np.reshape(
            mask, (mask.shape[0], mask.shape[1], self.nview, self.nclass), order="F"
        )
        maskiclass_canvas_a = np.transpose(
            maskiclass_canvas, [3, 2, 0, 1]
        )  # nclass, nview, H, W
    ...
    def read_mask_only(self):
        ...
        maskiclass_canvas = np.reshape(
            mask, (self.nclass, self.nview, mask.shape[1], mask.shape[2])
        )  # nclass, nview, H, W
