...
def extract_random(video_input, setupname, numframe_to_extract, maxlength):
    ...
    for iframe in tqdm.tqdm(range(length)):
        ...
        ipannel = random.randint(0, nviews - 1)  # 0-4,如果用frank就是nviews-2
        crop_xywh = views[ipannel]
        x, y, w, h = crop_xywh
        im1 = frame[y : y + h, x : x + w]
        filename = os.path.join(
            dirname, frame_dir, nakefilename + f"_pannel{ipannel}_{iframe:06}.jpg"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--setupname", type=str, default="ana", help="camera setup name"
    )
