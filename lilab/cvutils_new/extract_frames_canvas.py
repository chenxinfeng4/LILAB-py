...
def extract(video_input, numframe_to_extract, maxlength):
    ...
    np.random.seed(0)
    idxframe_to_extract = set(np.random.permutation(downsample_length)[
                              :numframe_to_extract]*frame_min_interval // 3 * 3)
    cap.release()
...
def ready_to_extract(video_input, idxframe_to_extract, rat_name_id=None, outdirname=None):
    ...
    rat_name_ids = [rat_name_id] if rat_name_id is not None else range(
        len(classnames))
...
def ready_to_extract_cv(video_input, idxframe_to_extract, rat_name_id=None):
    ...
    for iframe in tqdm.tqdm(idxframe_to_extract):
        ...
        for iclass in rat_name_ids:
            ...
            frame_out = frame_iclass[iclass]
            filename = os.path.join(
                dirname, frame_dir, nakefilename + '_{}_{:06}.jpg'.format(classname, iframe))
            cv2.imwrite(filename, frame_out, [
                        int(cv2.IMWRITE_JPEG_QUALITY), 100])
    cap.release()
