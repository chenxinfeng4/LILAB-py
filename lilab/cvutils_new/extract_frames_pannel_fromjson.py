...
def parser_json(json_file, dir_name=None):
    ...
    for vfile, pannel, iframeall in zip(tqdm.tqdm(vfiles, position=0), pannels, iframealls):
        full_vfile = os.path.join(dir_name, vfile)
        assert osp.exists(full_vfile), 'video_path not exists'
        vid = ffmpegcv.VideoCaptureNV(full_vfile, gpu=0, crop_xywh=crop_xywhs[int(pannel)])
        # make dir for each output pannel
        out_dir = osp.join(osp.dirname(full_vfile), '{}_pannel_{}'.format(osp.basename(full_vfile), pannel))
        os.makedirs(out_dir, exist_ok=True)
        iframeall = [int(iframe) for iframe in iframeall]
        iframemax = max(iframeall)
        with vid:
            for iframe, frame in enumerate(tqdm.tqdm(vid, position=1, desc=f'{pannel}')):
