...
dir_name = '/mnt/liying.cibr.ac.cn_Data_Temp/multiview_9/chenxf/shank3HetxWT_PND75/batch2/'
...
task_args = []
for video_nake, ipannel, idxframe_to_extract in zip(df_groupby['video_nake'],
            df_groupby['ipannel'], df_groupby['iframe']):
...
def ready_to_extract(video_input, idxframe_to_extract, dir_name, ipannel):
    ...
    gpu = GpuQuery().get()
    cap = ffmpegcv.VideoCaptureNV(video_input, gpu=gpu, crop_xywh=pannels_xywh[ipannel], pix_fmt='gray')
    ...
    for iframe in tqdm.tqdm(range(length), position=gpu):
        ...
        filename = os.path.join(dir_name, frame_dir, nakefilename + '_{:1}_{:06}.jpg'.format(ipannel, iframe))
